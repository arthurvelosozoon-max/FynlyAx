...
export type Card={id:string;name:string;limit:number;closingDay:number;dueDay:number};
export type Charge={id:string;cardId:string;description:string;amount:number;date:string;category:string};
...
export function purchaseSchedule(card:Card,date:string,amount:number,count:number,category:string,description:string,id:string):Charge[]{
 ...
 return splitInstallments(amount,count).map((value,i)=>({id:`${id}:${i+1}`,cardId:card.id,description:count>1?`${description.slice(0,105)} (${i+1}/${count})`:description,amount:value,date:monthDate(shiftMonth(dueMonth,i),card.dueDay),category}));
}
