...
test('PostgreSQL migration: RLS isolation, atomic revisions and validated writes',async()=>{
 ...
 try{
  ...
  await db.exec(await readFile(new URL('../../database/003_mvp_planning.sql',import.meta.url),'utf8'));
  await db.exec(await readFile(new URL('../../database/004_mvp_required_fields.sql',import.meta.url),'utf8'));
  assert.equal((await db.query<{valid:boolean}>("select public.fynlyax_valid_state($1::jsonb - 'transactions') as valid",[JSON.stringify(emptyState())])).rows[0].valid,false);
  ...
  assert.equal((await db.query<{ok:boolean}>('select public.fynlyax_take_ai_slot() as ok')).rows[0].ok,false);
  await assert.rejects(db.exec('select * from public.fynlyax_ai_usage'),(e:unknown)=>(e as {code:string}).code==='42501');
